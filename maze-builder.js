  ...
  constructor(height, width) {
    ...
    for (let i = 0; i < this.rows; i++) {
      ...
      for (let j = 0; j < this.cols; j++) {
        ...
        if (cell.length === 0)
          this.table[i].push(0)
        else if (cell.length === 1 && cell[0] == "wall")
          this.table[i].push(-1)
      }
    }
    for (key of this.keys) {
      this.table[key[0]][key[1]] = 1
    }
    this.table[this.doorPosIn[0]][this.doorPosIn[1]] = 2
    this.table[this.doorPosExit[0]][this.doorPosExit[1]] = 3
  }
